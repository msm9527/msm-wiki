...
### ✨ 新增（Added）
- 仪表盘新增流星图表动效与平滑插值更新
- 荣誉勋章支持 3D 展示及原生 GLB/STL 导出
- 新增 Sing-Box 代理核心及双核心无损切换
- 完善 Cloudflare Tunnel 多记录一等公民管理
- 新增受管反向代理运行时与域名服务闭环
- 增加原始股东勋章并优化监控空数据展示
- 完善系统更新与备份恢复功能
- 新增 Docker 独立子节点程序与管理界面
- 新增 Pro 工具模块开关与权限控制
...
- 统一远程访问与域名服务界面交互体验
- 优化仪表盘布局与组件动画性能表现
- 后端常驻内存优化采用紧凑 OUI 数据库
- 合并 Docker Center 与 Edge 正则首次编译
- 升级前端构建流程隔离避免开发页白屏
- 移除旧版 Cocoa API 同步 Tauri 依赖版本

### 🐛 修复（Fixed）
- 修复升级与托管进程恢复逻辑缺陷
- 解决代理核心互斥与运行时检测冲突
- 修正勋章 STL 内翻并提升打印精度细节
...
### 📝 备注（Notes）
- MSM 常驻内存显著降低，优化启动速度
- 发布构建使用隔离目录防止 Vite 缓存干扰
- Docker Agent 镜像版本号同步更新至 1.3.0
- 不再支持 Mesh 网关，专注 Tunnel 穿透
- 加固生产镜像与供应链安全
