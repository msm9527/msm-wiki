...
test('stable and beta releases gate publication on the standalone Docker Agent image', () => {
  ...
  assert.match(source, /MOVING_TAG="latest"/u)
  assert.match(source, /MOVING_TAG="beta-latest"/u)
  assert.match(source, /msmbox\/msm-agent:\$MOVING_TAG/u)
  assert.doesNotMatch(source, /msmbox\/msm:agent-/u)
