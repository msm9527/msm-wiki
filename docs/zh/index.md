...
[自建私网](/zh/guide/networking) 或 [远程回家](/zh/guide/home)
...
域名服务、Cloudflare 内网穿透、自建私网、Docker 管理和网络工具属于 Pro 扩展模块。入口是否显示由 **系统设置 → 功能开关** 控制；是否可以使用和执行高风险操作，还会同时检查 Pro 授权、当前用户角色及对应能力。
