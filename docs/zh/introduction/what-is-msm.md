...
MSM 是闭源、自托管的中性管理软件，不是网络接入服务、代理节点、订阅或内容提供者。它应仅用于用户自有或已获明确授权的设备、网络、域名和数据。

- 项目不为用户选择或审核订阅、节点、规则源和目标服务
- 用户应对配置、授权、数据处理、第三方服务和当地合规负责
- DNS、代理、隧道、端口检测和反向代理等能力不得用于未授权访问、侵害他人权益或其他违法活动

详细边界见 [使用与合规中心](/zh/legal/) 与 [合规使用规范](/zh/legal/acceptable-use)。

## 标准版与 Pro

MSM 自有代码采用闭源商业许可。标准版提供基础管理能力；Pro 通过许可证解锁高级模块、写操作、自动化能力或更高配额。购买 Pro 取得的是约定范围内的软件使用与升级服务，不取得 MSM 源码所有权，也不包含节点、订阅、带宽、公共 IP、网络线路或中继服务。

Pro 官方入口见 [Pro 购买与授权](/zh/legal/pro-and-support)，激活和设备迁移见[授权管理](/zh/guide/license)。
