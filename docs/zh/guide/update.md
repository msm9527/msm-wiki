...
安装前 MSM 会校验下载文件哈希，并使用 `--version` 对候选二进制做可执行性与版本预检。只有候选文件通过检查后才替换当前程序；替换或重启失败时会保留回滚路径，不会把 HTML 错误页、截断文件或错误架构产物直接安装为 MSM。
...
如果 MSM 由镜像部署，Web 页面会拒绝 MSM 本体二进制自更新，并提示修改镜像版本后重建容器。这是为了避免只改容器可写层、下次重建又回到旧版本。使用原 Compose 文件：
